...
import { ArrowUpRight, Award, Globe2, HeartHandshake, ShieldCheck, Truck, Users } from "lucide-react";
import { equestrianCategories, products, veterinaryCategories } from "@/data/products";
import HeroSlider from "@/components/HeroSlider";

const stats = [
  { value: "65+", label: "Years of craft", icon: Award },
  { value: "40+", label: "Countries served", icon: Globe2 },
  { value: "500+", label: "Product varieties", icon: ShieldCheck },
...
export default function HomePage() {
  ...
  return <>
    <HeroSlider />
    <div className="ticker-band"><div className="ticker-track"><span>PRECISION INSTRUMENTS</span><b>✦</b><span>MADE IN SIALKOT</span><b>✦</b><span>GLOBAL PARTNERSHIPS</span><b>✦</b><span>PRECISION INSTRUMENTS</span><b>✦</b><span>MADE IN SIALKOT</span><b>✦</b></div></div>
    <section className="stats-strip"><div className="page-wrap stats-grid">{stats.map(({ value, label, icon: Icon }) => <div className="stat-item" key={label}><Icon size={20} strokeWidth={1.5} /><strong>{value}</strong><span>{label}</span></div>)}</div></section>
