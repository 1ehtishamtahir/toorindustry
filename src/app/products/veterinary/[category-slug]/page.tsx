...
export function generateStaticParams() {
  return veterinaryCategories.map((cat) => ({
...
export default async function VeterinaryCategoryPage({ params }: Props) {
  const { "category-slug": slug } = await params;
  const category = getCategoryBySlug(slug);
  ...
  return (
    <>
      <section className="inner-hero inner-hero-veterinary">
        <div className="page-wrap">
          <nav style={{ fontSize: 12, color: "#94a3b8", marginBottom: 16 }}>
            ...
            <Link href="/products/veterinary" style={{ color: "#94a3b8", transition: "color .3s" }}>
              Veterinary
            </Link>
            <span style={{ margin: "0 8px" }}>/</span>
          ...
          <h1>{category.name.split(" ").slice(0, 2).join(" ")}<br /><em>{category.name.split(" ").slice(2).join(" ") || "Instruments."}</em></h1>
          ...
          <div className="product-grid" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))" }}>
            {categoryProducts.map((product) => (
              <div key={product.slug} style={{ display: "flex", flexDirection: "column" }}>
                <Link
                  href={`/products/veterinary/${slug}/${product.slug}`}
